...
class Alloy(object):
    """ This class contains commodities for the Alloy specification language. """
    ...
    # Following dictionary stores the extends relation between Alloy signatures.
    the_extends_relation_between_signatures = {}

    def declare_signature(signature_name, extends_signature_name = None):
        """ Declare that an Alloy signature extends another Alloy signature. """
        Alloy.the_extends_relation_between_signatures[signature_name] = extends_signature_name

    def is_signature_declared(signature_name):
        """ Is an Alloy signature declared. """
        return Alloy.the_extends_relation_between_signatures.get(signature_name) != None
...
class AlloyCommandScope(object):
    ...
    def __init__(self, type_system):
        ...
        for type_name, type_yaml in Alloy.the_extends_relation_between_signatures.items():
...
class AbstractAlloySigGenerator(Generator):
    ...
    def get_implementation_artifact_type(self, implementation):
        ...
        if index != -1:
            result = self.type_system.get_artifact_type_by_file_ext(implementation[index+1:])
        if result == None:
            self.warning(" no implementation artifact type associated to implementation '" + implementation + "'")
            ...
            if node_type_yaml:
                ...
                if items:
                    ...
                    if items.get(item_name):
                        return True
                return self.is_defined(node_type_yaml.get(DERIVED_FROM), item_kind, item_name, normalize)
        return False
    ...
    def generate_occurrences_cardinality_constraints(self, relation_name, yaml, minimum):
        occurrences = syntax.get_occurrences(yaml)
        if occurrences:
    ...
    def generate_sig_field(self, prefix, field_name, field_cardinality, field_type):
        self.generate('  ', self.prefix_name(prefix, field_name), ': ', field_cardinality, ' ', field_type, ',', sep='')
    ...
    def stringify_value(self, value, value_type_definition, context_error_message, list_sep = ' + '):
        if type(value) == list:
            ...
            index = 0
            for v in value:
                s = self.stringify_value(v, value_type_definition, context_error_message + '[' + str(index) + ']')
                if result:
    ...
    def generate_property(self, prefix, property_value, property_declaration, context_error_message):
        ...
        if property_type == 'integer': # special case to generate a comment
            ...
            if type(property_value) == int and property_value >= MAX_INT:
                comment = ' ' + Alloy.SINGLE_LINE_COMMENT + ' ISSUE: ' + str(property_value) + ' is a too big integer!!!'
            self.generate('  ', prefix, ' = ', self.stringify_value(property_value, property_declaration, context_error_message), comment, sep='')
            ...
            if type(property_value) == dict:
                ...
                self.generate('  ', prefix, '.init[', self.stringify_value(property_value, property_declaration, context_error_message), ']', sep='')
            ...
            for value in property_value:
                tmp = '[' + str(index) + ']'
                self.generate_property(prefix + tmp, value, entry_schema, context_error_message + tmp)
                index = index + 1
                ...
                if len(property_value):
                    ...
                    for key, value in property_value.items():
                        ...
                        prefixed_entry = '(' + entry_cast_sig + '<:(' + prefix + '.entry["' + key + '"]))'
                        self.generate_property(prefixed_entry, value, entry_schema, context_error_message + ':' + key)
                        if keys:
                            keys = keys + ' + '
                        keys = keys + '"' + key + '"'
            ...
            if type_type:
                ...
                if self.type_system.is_yaml_type(derived_from):
                    self.generate_property(prefix, property_value, { syntax.TYPE: derived_from }, context_error_message)
                    return
    ...
    def generate_all_properties(self, all_declared_properties, template_properties, prefixed_template_name, context_error_message, property_name_format = 'property_%s', generate_no_value=True):
        ...
        for property_name, property_yaml in template_properties.items():
            if not all_declared_properties.get(property_name):
                self.error(context_error_message + ": property '" + property_name + "' unknown")

        prefix = prefixed_template_name + '.'

        for (property_name, property_declaration) in all_declared_properties.items():
            prefixed_property_name = prefix + self.toscaProperty2alloyField(property_name_format % utils.normalize_name(property_name))
            ...
            if property_value != None:
                ...
                self.generate_property(prefixed_property_name, property_value, property_declaration, context_error_message + ':' + property_name)
            else:
                if is_property_required(property_declaration):
                    ...
                    property_default = syntax.get_property_default(property_declaration)
                    if property_default:
                        self.generate("  // NOTE: The property '", property_name, "' is not required but has a default value.", sep='')
                        self.generate_property(prefixed_property_name, property_default, property_declaration, context_error_message + ':' + property_name)
                    else:
                        if generate_no_value:
                            self.generate("  // NOTE: The property '", property_name, "' is not required.", sep='') # and has no default value
        ...
        if constraints:
            for constraint in constraints:
                for constraint_name, constraint_yaml in constraint.items():
                    ctx_error_msg = context_error_message + ':constraints:' + constraint_name
...
# TBR                self.generate('  ', name, '.entry_schema_type[', utils.normalize_name(entry_schema_yaml.get(TYPE)), ']', sep='')
            self.generate_constraints_facts(name + '.elems', entry_schema_yaml, context_error_message + ':entry_schema')

    def generate_parameter_facts(self, indentation, prefix, parameter_name, parameter_yaml, context_error_message):
        ...
        if parameter_description:
            ...
            self.generate(indentation, prefix, parameter_name, '.description = "', parameter_description, '"', sep='')
        else:
            self.generate(indentation, 'no ', prefix, parameter_name, '.description', sep='')

        parameter_value = syntax.get_input_value(parameter_yaml)
        if parameter_value:
            self.generate(indentation, '// YAML value: ', parameter_value, sep='')
            self.generate_property(indentation[:len(indentation)-2] + prefix + parameter_name + '.value', parameter_value, parameter_yaml, context_error_message + ':value')

        parameter_required = syntax.is_property_required(parameter_yaml)
        self.generate(indentation, '// YAML required: ', str(parameter_required).lower(), sep='')
        self.generate(indentation, prefix, parameter_name, '.required[', str(parameter_required).lower(), ']', sep='')
        ...
        if parameter_default:
            property_name = indentation[:len(indentation)-2] + '(' + self.alloy_sig(parameter_type) + '<:' + prefix + parameter_name + '.default)'
            self.generate_property(property_name, parameter_default, parameter_yaml, context_error_message + ':default')
        else:
            self.generate(indentation, 'no ', prefix, parameter_name, '.default', sep='')

        parameter_status = syntax.get_input_status(parameter_yaml)
        self.generate(indentation, '// YAML status: ', parameter_status, sep='')
        self.generate(indentation, prefix, parameter_name, '.status = "', parameter_status, '"', sep='')

        self.generate_constraints_facts(indentation[:len(indentation)-2] + prefix + parameter_name + '.value', parameter_yaml, context_error_message + ':' + parameter_name)

        parameter_external_schema = syntax.get_input_external_schema(parameter_yaml)
        if parameter_external_schema:
            self.generate(indentation, '// YAML external_schema: ', parameter_external_schema, sep='')
            self.generate(indentation, prefix, parameter_name, '.external_schema = "', parameter_external_schema, '"', sep='')
        else:
            self.generate(indentation, 'no ', prefix, parameter_name, '.external_schema', sep='')
        ...
        if parameter_metadata:
            ...
            self.generate(indentation, '#', prefix, parameter_name, '.metadata = ', len(parameter_metadata), sep='')
            for metadata_name, metadata_value in parameter_metadata.items():
              self.generate(indentation, prefix, parameter_name, '.metadata["', metadata_name, '"] = "', metadata_value, '"', sep='')
        else:
            self.generate(indentation, 'no ', prefix, parameter_name, '.metadata', sep='')
...
# self.generate_command_Check(name, yaml)
    ...
    def generate_command_Show(self, name, yaml):
        ...
        self.generate('  // NOTE: Setting following scopes strongly reduces the research space.')
        for (sig_name, sig_scope) in self.get_command_Show_LG_scopes(name, yaml).items():
            self.generate('  ', Alloy.EXACTLY, ' ', sig_scope, ' ', sig_name, ',', sep='')
    ...
    def generate_command_Check_predicates_for_naming(self, yaml, keyword, predicate):
        ...
        if items:
            ...
            for (item_name, item_yaml) in utils.normalize_dict(items).items():
                ...
                self.generate('    instance.', prefixed_item_name, '.name = "', item_name, '"', sep='')
                self.generate('    instance.', predicate, '["', item_name, '"] = instance.', prefixed_item_name, sep='')
...
class AbstractTypeGenerator(AbstractAlloySigGenerator):
    ...
    def generate_sig(self, type_name, type_yaml):
        ...
        if self.type_system.is_yaml_type(derived_from):
            self.generate(Alloy.LET, self.alloy_sig(type_name), '=', self.alloy_sig(derived_from))
            return
        self.generate(Alloy.SIG, self.alloy_sig(type_name), Alloy.EXTENDS, self.alloy_sig(derived_from))
        self.generate('{')
    ...
    def generate_attributes_fields(self, type_name, type_yaml):
        ...
        if attributes:
            ...
            for (attribute_name, attribute_yaml) in attributes.items():
                self.generate('  // YAML ', attribute_name, ': ', attribute_yaml, sep='')
        ...
        if properties:
            ...
            for (property_name, property_yaml) in properties.items():
                if type(property_yaml) == dict:
                    self.generate_constraints_facts(self.prefix_name(None, property_name), property_yaml, self.get_kind() + '_types:' + type_name + ':properties:' + property_name)
            self.generate()
...
#        return
    ...
    def generate_facts(self, artifact_type_name, artifact_type_yaml):
        AbstractTypeGenerator.generate_facts(self, artifact_type_name, artifact_type_yaml)
...
class InterfaceTypeGenerator(AbstractTypeGenerator):
    ...
    def generate_fields(self, interface_type_name, interface_type_yaml):
        # Generate properties
        AbstractTypeGenerator.generate_fields(self, interface_type_name, interface_type_yaml)
    ...
    def generate_facts(self, interface_type_name, interface_type_yaml):
        # Generate properties
        AbstractTypeGenerator.generate_facts(self, interface_type_name, interface_type_yaml)
        ...
        if interface_type_yaml:
            ...
            for (operation_name, operation_yaml) in syntax.get_operations(interface_type_yaml).get(OPERATIONS).items():
                    ...
                    self.generate_call_predicate(prefixed_operation_name + '.name', '"' + operation_name + '"')
...
#                    self.generate_inputs_facts(prefixed_operation_name + '.@', operation_yaml, INTERFACE_TYPES + ':' + interface_type_name + ':' + operation_name)
    ...
    def generate_command_Check_predicates(self, interface_type_name, interface_type_yaml):
        for (operation_name, operation_yaml) in interface_type_yaml.items():
            self.generate('    instance.', self.prefix_name('operation', operation_name), '.name = "', operation_name, '"', sep='')
            self.generate('    instance.operation["', operation_name, '"] = instance.', self.prefix_name('operation', operation_name), sep='')
    ...
    def generate_fields(self, name, yaml):
        ...
        if interfaces:
            ...
            for (interface_name, interface_yaml) in interfaces.items():
                self.generate('  // YAML ', interface_name, ': ', interface_yaml, sep='')
                if self.is_interface_defined(yaml.get(DERIVED_FROM), interface_name):
                    ...
                    if interface_type:
                        self.generate_sig_field('interface', interface_name, Alloy.ONE, self.alloy_sig(interface_type))
                self.generate()
...
#        return
    ...
    def generate_facts(self, relationship_type_name, relationship_type_yaml):
        ToscaComponentTypeGenerator.generate_facts(self, relationship_type_name, relationship_type_yaml)
...
#        return
    ...
    def get_command_Show_LG_scopes(self, relationship_type_name, relationship_type_yaml):
...
class NodeTypeGenerator(ToscaComponentTypeGenerator):
    ...
    def generate_fields(self, node_type_name, node_type_yaml):
        ToscaComponentTypeGenerator.generate_fields(self, node_type_name, node_type_yaml)
        ...
        if capabilities:
            ...
            for (capability_name, capability_yaml) in capabilities.items():
                ...
                if self.is_capability_defined(node_type_yaml.get(DERIVED_FROM), capability_name):
                    self.generate(' ', Alloy.SINGLE_LINE_COMMENT, capability_name, 'overloaded')
                else:
                    if type(capability_yaml) == str:
                        ...
                        self.error('Invalid capability declaration ' + capability_name + ': ' + capability_yaml)
                        capability_type = TOSCA.Capability
                        capability_cardinality = Alloy.ONE
                    self.generate_sig_field('capability', capability_name, capability_cardinality, self.alloy_sig(capability_type))
                self.generate()
        ...
        if requirements:
            ...
            for (requirement_name, requirement_yaml) in requirements.items():
                self.generate('  // YAML ', requirement_name, ': ', requirement_yaml, sep='')
                if self.is_requirement_defined(node_type_yaml.get(DERIVED_FROM), requirement_name):
                    self.generate(' ', Alloy.SINGLE_LINE_COMMENT, requirement_name, 'overloaded')
                else:
                    self.generate_description(requirement_yaml, '  ')
                    requirement_type = None # syntax.get_requirement_capability(requirement_yaml)
                    if not requirement_type:
                        requirement_type = TOSCA.Requirement
                    requirement_cardinality = self.get_requirement_cardinality(requirement_yaml)
                    self.generate_sig_field('requirement', requirement_name, requirement_cardinality, self.alloy_sig(requirement_type))
                self.generate()
        ...
        if artifacts:
            ...
            for (artifact_name, artifact_yaml) in artifacts.items():
                ...
                if self.is_artifact_defined(node_type_yaml.get(DERIVED_FROM), artifact_name):
                    self.generate('  // NOTE:', artifact_name, 'overloaded')
                else:
                    self.generate_sig_field('artifact', artifact_name, artifact_cardinality, self.alloy_sig(artifact_type))
                self.generate()
        ...
        if capabilities:
            ...
            for (capability_name, capability_yaml) in capabilities.items():
                ...
                if type(capability_yaml) == dict:
                    self.generate_description(capability_yaml, '  ')
                self.generate('  // YAML ', capability_name, ': ', capability_yaml, sep='')
                capability_alloy_field_name = self.prefix_name('capability', capability_name)
                if not self.is_capability_defined(node_type_yaml.get(DERIVED_FROM), capability_name):
                    self.generate_call_predicate(capability_alloy_field_name + '.name', '"' + capability_name + '"')
                    self.generate_call_predicate('capability', capability_alloy_field_name)
                if type(capability_yaml) == dict:
                    ...
                    if valid_source_types:
                        ...
                        self.generate_call_predicate(capability_alloy_field_name + '.valid_source_types', tmp)
#                    else:
#                        self.error(node_type_name + ':' + capability_name + ': valid_source_types must list one or more node types')
                    self.generate_occurrences_cardinality_constraints(capability_alloy_field_name, capability_yaml, 1)
                self.generate()
        ...
        if requirements:
            ...
            requirements = utils.normalize_dict(requirements)
            for (requirement_name, requirement_yaml) in requirements.items():
                if requirement_yaml != None:
                    self.generate('  // YAML ', requirement_name, ': ', requirement_yaml, sep='')
                    self.generate_description(requirement_yaml, '  ')
                    prefixed_requirement_name = self.prefix_name('requirement', requirement_name)
                    if not self.is_requirement_defined(node_type_yaml.get(DERIVED_FROM), requirement_name):
                        self.generate('  requirement["', requirement_name, '", ', prefixed_requirement_name, ']', sep='')
                    ...
                    if requirement_capability:
                        ...
                        self.generate('  ', prefixed_requirement_name, ".capability[", self.alloy_sig(requirement_capability), ']', sep='')
                    ...
                    if requirement_relationship_type:
                        # TODO generate_call_predicate
                        self.generate('  ', prefixed_requirement_name, ".relationship[", self.alloy_sig(requirement_relationship_type), ']', sep='')
                    requirement_node = get_requirement_node_type(requirement_yaml)
                    if requirement_node:
                        # TODO generate_call_predicate
                        self.generate('  ', prefixed_requirement_name, ".node[", self.alloy_sig(requirement_node), ']', sep='')
                    self.generate_occurrences_cardinality_constraints(prefixed_requirement_name, requirement_yaml, 0)
                    self.generate()
        ...
        if artifacts:
            ...
            for (artifact_name, artifact_yaml) in artifacts.items():
                ...
                self.generate_call_predicate(prefixed_artifact_name + '.name', '"' + artifact_name + '"')
...
#                   TOSCA.Operation: 27, # TODO: compute exact scope
                ...
                if requirement_yaml != None:
                    if self.get_requirement_cardinality(requirement_yaml) == Alloy.ONE:
                        ...
                        if requirement_node != None:
                            result = result + self.get_required_locations(requirement_node)
    ...
    def generate_command_Check_predicates(self, node_type_name, node_type_yaml):
        # Check capabilities
        self.generate_command_Check_predicates_for_naming(node_type_yaml, CAPABILITIES, 'capability')
        # Check requirements
        self.generate_command_Check_predicates_for_naming(node_type_yaml, REQUIREMENTS, 'requirement')
        # Check artifacts
        self.generate_command_Check_predicates_for_naming(node_type_yaml, ARTIFACTS, 'artifact')
        # TODO: are other predicates to generate?
        return
...
class GroupTypeGenerator(ToscaComponentTypeGenerator):
    ...
    def generate_fields(self, group_type_name, group_type_yaml):
        # Generate attributes and interfaces
        ToscaComponentTypeGenerator.generate_fields(self, group_type_name, group_type_yaml)
        # TODO: are other facts to generate?
        return

    def generate_facts(self, group_type_name, group_type_yaml):
        # Generate attributes and interfaces
        ToscaComponentTypeGenerator.generate_facts(self, group_type_name, group_type_yaml)
...
class PolicyTypeGenerator(ToscaComponentTypeGenerator):
    ...
    def generate_fields(self, policy_type_name, policy_type_yaml):
        # Generate attributes and interfaces
        ToscaComponentTypeGenerator.generate_fields(self, policy_type_name, policy_type_yaml)
        # TODO: are other facts to generate?
        return

    def generate_facts(self, policy_type_name, policy_type_yaml):
        # Generate attributes and interfaces
        ToscaComponentTypeGenerator.generate_facts(self, policy_type_name, policy_type_yaml)
...
class TopologyTemplateGenerator(AbstractAlloySigGenerator):
    ...
    def generate_all_sigs(self):
        topology_template_yaml = self.tosca_service_template.get_yaml().get('topology_template')
        ...
        topology_template_sig_name = utils.normalize_name(path[:path.rfind('.')]) + '_topology_template'
    ...
    def generate_template_field(self, template_name, template_yaml, type_keyword, field_prefix, context_message):
        self.generate('  // YAML ', template_name, ': ', template_yaml, sep='')
        template_yaml_type = template_yaml.get(type_keyword)
        if template_yaml_type == None:
            self.error(TOPOLOGY_TEMPLATE + ':' + context_message + ':' + template_name + ': ' + type_keyword + ' keyword missed')
            self.generate("  // ERROR: '", type_keyword, "' keyword missed!", sep='')
            return
        if self.type_system.get_type(template_yaml_type) == None:
            self.error(TOPOLOGY_TEMPLATE + ':' + context_message + ':' + template_name + ':' + type_keyword + ': ' + template_yaml_type + ' undefined')
            self.generate('  // ERROR: ', type_keyword, ' ', template_yaml_type, ' undefined', sep='')
            return
        self.generate('  // YAML ', type_keyword, ': ', template_yaml_type, sep='')
        self.generate_sig_field(field_prefix, template_name, Alloy.ONE, self.alloy_sig(template_yaml_type))
        self.generate()
    ...
    def generate_template_fields(self, keyword, topology_template, field_prefix):
        ...
        for (template_name, template_yaml) in templates.items():
            self.generate_template_field(template_name, template_yaml, TYPE, field_prefix, keyword)
    ...
    def generate_fields(self, sig_name, topology_template_yaml):
        ...
        self.generate_template_fields(RELATIONSHIP_TEMPLATES, topology_template_yaml, 'relationship')
        ...
        if substitution_mappings:
            ...
            self.generate_template_field(SUBSTITUTION_MAPPINGS, substitution_mappings, NODE_TYPE, None, '')
    ...
    def generate_interfaces_facts(self, template_name, template_yaml, prefixed_template_name, merged_template_type, context_error_message):
        ...
        self.generate_cardinality_fact(prefixed_template_name + '.' + INTERFACES, len(interfaces))
        for (interface_name, interface_yaml) in interfaces.items():
...
# TODO: inputs must be generated!
    ...
    def generate_templates_facts(self, topology_template, keyword, relation_name, predicate_name, field_prefix, more_generation):
        # Translate templates.
        self.generate_header('YAML ' + keyword + ':', '  ')
        ...
        for (template_name, template_yaml) in templates.items():
            ...
            self.generate_all_properties(get_dict(merged_template_type, PROPERTIES),
                                         get_dict(template_yaml, PROPERTIES),
                                         prefixed_template_name,
                                         TOPOLOGY_TEMPLATE + ':' + keyword + ':' + template_name + ':' + PROPERTIES)

            # Constraints the cardinality of interfaces, operations and inputs
            # and set operation implementations
            # and set operation inputs
            self.generate_interfaces_facts(template_name, template_yaml, prefixed_template_name, merged_template_type, TOPOLOGY_TEMPLATE + ':' + keyword + ':' + template_name)

            if more_generation:
                more_generation(self, template_name, template_yaml, merged_template_type, prefixed_template_name, TOPOLOGY_TEMPLATE + ':' + keyword + ':' + template_name)
            self.generate()
    ...
    def generate_facts(self, name, topology_template_yaml):
        ...
        def generate_artifacts_capabilities_requirements(self, node_template_name, node_template_yaml, merged_node_template_type, prefixed_node_template_name, context_error_message):
            ...
            for artifact_name, artifact_yaml in artifacts.items():
                self.generate('  // YAML   ', artifact_name, ': ', artifact_yaml, sep='')
                if artifact_yaml == None:
                    artifact_yaml = {}
                artifact_type = syntax.get_artifact_type(artifact_yaml)
                if artifact_type == None:
                    self.warning(context_error_message + ':' + ARTIFACTS + ':' + artifact_name + ' must have a type')
                    artifact_type = TOSCA.Artifact
                prefixed_artifact_name = self.prefix_name('artifact', artifact_name)
                self.generate('  one',prefixed_artifact_name, ':', self.alloy_sig(artifact_type), '{')
                self.generate('    ', prefixed_node_template_name, '.artifact[', prefixed_artifact_name, ']', sep='')
                self.generate('    ', prefixed_artifact_name, '.name["', artifact_name, '"]', sep='')
            ...
            for capability_name, capability_yaml in capabilities.items():
                if not all_capabilities.get(capability_name):
                    self.error(context_error_message + ':' + CAPABILITIES + ':' + capability_name + ': undefined capability')
            ...
            for capability_name, capability_yaml in all_capabilities.items():
                ...
                self.generate('  // YAML   ', capability_name, ': ', capability_value, sep='')
                if capability_yaml:
                    prefixed_capability_name = prefixed_node_template_name + '.' + self.prefix_name('capability', capability_name)
                    if capability_value == None:
                        capability_value = {}
                    merged_capability_type = self.type_system.merge_type(get_capability_type(capability_yaml))
                    self.generate_all_properties(get_dict(merged_capability_type, PROPERTIES),
            ...
            for requirement in requirements:
                for requirement_name, requirement_yaml in requirement.items():
                    # ACK for Alien4Cloud
                    requirement_name = syntax.get_type_requirement(requirement_yaml, requirement_name)
                    if not all_requirements.get(requirement_name):
                        self.error(context_error_message + ':' + requirement_name + ' - requirement undefined')
            ...
            relationship_templates = get_dict(topology_template_yaml, RELATIONSHIP_TEMPLATES)
            ...
            for requirement in requirements:
                for (requirement_name, requirement_yaml) in requirement.items():
                    ...
                    requirement_node_filter = get_requirement_node_filter(requirement_yaml)
                    if requirement_node == None and requirement_node_filter == None:
                        self.error(context_error_message + ':' + REQUIREMENTS + ':' + requirement_name + ' - node or node_filter expected')
                        continue
                    if requirement_node != None and requirement_node_filter != None:
                    ...
                    if requirement_node_yaml == None:
                         self.error(context_error_message + ':' + REQUIREMENTS + ':' + requirement_name + ':node: ' + requirement_node + ' - undefined node template')
                         continue
                    requirement_node_type_name = requirement_node_yaml.get(TYPE)
                    if requirement_node_type_name == None:
                        continue
                    merged_requirement_node_type = self.type_system.merge_node_type(requirement_node_type_name)
                    if merged_requirement_node_type == None:
                        self.error(context_error_message + ':' + REQUIREMENTS +':' + requirement_name + ": node type '" + node_templates.get(requirement_node).get(TYPE) + "' unknown")
                        continue
                    requirement = all_requirements.get(requirement_name)
                    requirement_capability = syntax.get_requirement_capability(requirement)
                    capability_found = False
                    for ( capability_name, capability_yaml ) in get_dict(merged_requirement_node_type, CAPABILITIES).items():
                        if self.type_system.is_derived_from(get_capability_type(capability_yaml), requirement_capability):
                            capability_found = True
                            break
                    if capability_found:
                        prefixed_requirement_name = self.prefix_name('requirement', requirement_name)
                        prefixed_capability_name = self.prefix_name('capability', capability_name)
                        self.generate('  connect[', prefixed_node_template_name, '.', prefixed_requirement_name, ', ', utils.normalize_name(requirement_node), '.', prefixed_capability_name, ']', sep='')
                    else:
                        if requirement_node_type_name == None:
                             requirement_node_type_name = 'unknown node type'
                        if requirement_capability == None:
                            requirement_capability = 'unknown'
                        self.error(context_error_message + ':' + REQUIREMENTS + ':' + requirement_name + ": capability of type " + requirement_capability + " not found in " + requirement_node_type_name)
                        prefixed_requirement_name ='unknown'

                    requirement_relationship_properties = {}
                    merged_requirement_relationship_type = {}
                    requirement_relationship = syntax.get_requirement_relationship(requirement)
                    requirement_relationship_type = syntax.get_relationship_type(requirement_relationship)
                    ...
                    prefixed_relationship = prefixed_node_template_name + '.' + prefixed_requirement_name + '.relationship'
                    ...
                    if type(requirement_yaml) == dict:
                        ...
                        if type(requirement_relationship) == dict:
                            ...
                            merged_requirement_relationship_type = self.type_system.merge_node_type(requirement_relationship_type)
                            requirement_relationship_properties = {}
                            self.generate('  ', prefixed_relationship, '[', utils.normalize_name(requirement_relationship_type), ']', sep='')
                    self.generate('  ', prefixed_relationship, '._name_ = "(anonymous)"', sep='')
                    ...
                    self.generate_all_properties(
                                 ...
                                 context_error_message + ':' + REQUIREMENTS + ':' + requirement_name,
                                 property_name_format='property_%s')

                    self.generate_interfaces_facts(node_template_name, { TYPE : requirement_relationship_type }, prefixed_relationship, merged_requirement_relationship_type, context_error_message + ':' + REQUIREMENTS + ':' + requirement_name)

        self.generate_templates_facts(topology_template_yaml, NODE_TEMPLATES, 'nodes', 'node', None, generate_artifacts_capabilities_requirements)

        self.generate()

        # Generate facts for relationships
        self.generate_templates_facts(topology_template_yaml, RELATIONSHIP_TEMPLATES, 'relationships', 'relationship', 'relationship', None)

        self.generate()

        # Generate facts for groups

        def generate_members(self, group_name, group_yaml, merged_group_type, prefixed_group_name, context_message):
            # Generate members.
        ...
        self.generate_templates_facts(topology_template_yaml, GROUPS, 'groups', 'group', 'group', generate_members)

        self.generate()

        # Generate facts for policies

        def generate_targets(self, policy_name, policy_yaml, merged_policy_type, prefixed_policy_name, context_message):
            ...
            if targets:
                ...
                for target in targets:
                    if all_node_templates.get(target):
                        ...
                        self.error(context_message + ':targets[' + str(idx) + '] - ' + target + ' node template or group undefined')
                        idx = idx + 1
                        continue
                    if tmp != '':
                        tmp = tmp + ' + '
        ...
        self.generate_templates_facts(topology_template_yaml, POLICIES, 'policies', 'policy', 'policy', generate_targets)
        ...
        for (output_name, output_yaml) in outputs.items():
            ...
            self.generate('  output_', output_name, '.name["', output_name, '"]', sep='')
            self.generate_parameter_facts('  ', 'output_', output_name, output_yaml, TOPOLOGY_TEMPLATE + ':' + ':' + OUTPUTS + ':' + output_name)
            self.generate()
        ...
        if substitution_mappings == None:
            ...
            if len(requirements) > 0:
                ...
                for requirement_name, requirement_yaml in requirements.items():
                    self.generate('  // YAML     ', requirement_name, ': ', requirement_yaml, sep='')
                    if requirement_yaml:
                        self.generate('  connectRequirement[',
                                      'substitution_mappings.',
    ...
    def merge_node_template_definition(self, node_template_yaml):
        ...
        if node_template_interfaces:
            ...
            if interfaces:
                result[INTERFACES] = utils.merge_dict(interfaces, node_template_interfaces)
            else:
        ...
        if property_type == 'integer':
            ...
            if property_value:
                ...
                for elem in property_value:
                    self.compute_scope_property(acs, property_declaration.get(ENTRY_SCHEMA), elem)
            ...
            if not self.type_system.is_yaml_type(derived_from):
                ...
                if type_type and type(property_value) == dict:
                    self.compute_scope_properties(acs, get_dict(type_type, PROPERTIES), property_value)

    def compute_scope_properties(self, acs, all_declared_properties, template_properties):
        if template_properties == None:
            template_properties = {}
        ...
        for (property_name, property_declaration) in all_declared_properties.items():
            ...
            if property_value != None:
                ...
                property_default = syntax.get_property_default(property_declaration)
                if is_property_required(property_declaration):
                    self.compute_scope_property(acs, property_declaration, property_default)
                else:
                    if property_default:
                        self.compute_scope_property(acs, property_declaration, property_default)

    def compute_scope_interfaces(self, acs, node_type, node_template_type, node_template):
...
#                        self.compute_scope_property(acs, input_yaml, node_template.get(INTERFACES,{}).get(operation_name,{}).get(input_name))
    ...
    def compute_scope_templates(self, acs, topology_template, keyword, more_computation):
        # Iterate over all templates.
        for template_name, template_yaml in get_dict(topology_template, keyword).items():
            ...
            if more_computation:
                more_computation(self, acs, template_name, template_yaml, merged_template_type, TOPOLOGY_TEMPLATE + ':' + keyword + ':' + template_name)

    def compute_scope_topology_template(self, acs, topology_template_name, topology_template_yaml):
        ...
        for input_name, input_yaml in inputs.items():
            ...
            if input_yaml.get(TYPE) == None:
                input_yaml[TYPE] = 'string'
            self.compute_scope_property(acs, input_yaml, {})

        # Iterate over all node templates.

        def compute_scope_capabilities_requirements(self, acs, node_template_name, node_template_yaml, merged_node_template_type, context_message):
            all_relationship_templates = get_dict(topology_template_yaml, RELATIONSHIP_TEMPLATES)
            ...
            for capability_name, capability_yaml in get_dict(merged_node_template_type, CAPABILITIES).items():
                ...
                capability_lower_occurrences = get_capability_occurrences(capability_yaml)[0]
                if capability_lower_occurrences:
                    acs.update_sig_scope(self.alloy_sig(capability_type), capability_lower_occurrences)
            ...
            if node_template_requirements == None: # TODO: Factorize this code pattern.
        ...
        self.compute_scope_templates(acs, topology_template_yaml, RELATIONSHIP_TEMPLATES, None)
        ...
        if substitution_mappings:
            ...
            for (requirement_name, requirement_yaml) in get_dict(merged_node_type_declaration, REQUIREMENTS).items():
                ...
                requirement_lower_occurrences = get_requirement_occurrences(requirement_yaml)[0]
                unfilled_requirements = requirement_lower_occurrences - nb_filled_requirements
                if unfilled_requirements > 0:
                    acs.update_sig_scope(TOSCA.Requirement, unfilled_requirements)

            # Compute the scope for node template interfaces.
            self.compute_scope_interfaces(acs, node_type, merged_node_type_declaration, substitution_mappings)
    ...
    def generate_commands(self, topology_template_name, topology_template_yaml):
        ...
        self.compute_scope_topology_template(acs, topology_template_name, topology_template_yaml)

        # Generate the Show command.
        self.generate('/** There exists some', topology_template_name, '*/')
        self.generate(Alloy.RUN, ' Show_', utils.normalize_name(topology_template_name), ' {', sep='')
        self.generate('}', Alloy.FOR, 0, Alloy.BUT)
        self.generate('  // NOTE: Setting following scopes strongly reduces the research space.')
        for (sig_name, sig_scope) in acs.get_signature_scopes().items():
            self.generate('  ', Alloy.EXACTLY, ' ', sig_scope, ' ', sig_name, ',', sep='')
        self.generate('  ', self.configuration.get(ALLOY, SCOPE, 'Int'), ' ', Alloy.Int, ',', sep='')
        ...
        for import_definition in imports:
            ...
            try:
                imported_file_yaml = self.tosca_service_template.imports(filepath).get_yaml()
        ...
        if generate_Substitute_command:
            ...
            self.generate(Alloy.RUN, ' Substitute_', utils.normalize_name(topology_template_name), ' {', sep='')
            self.generate('  apply_substitution[', utils.normalize_name(topology_template_name), ']', sep='')
            self.generate('}', Alloy.FOR, 0, Alloy.BUT)
            self.generate('  // NOTE: Setting following scopes strongly reduces the research space.')
            for (sig_name, sig_scope) in acs.get_signature_scopes().items():
                self.generate('  ', Alloy.EXACTLY, ' ', sig_scope, ' ', sig_name, ',', sep='')
...
class AlloyGenerator(AbstractAlloySigGenerator):
    ...
    def generation(self):
        ...
        for metadata_name in [ TOSCA_DEFINITIONS_VERSION, DESCRIPTION ]:
            ...
            if metadata_value:
                self.generate(Alloy.SINGLE_LINE_COMMENT + ' ', metadata_name, ': ', metadata_value, sep='')
        self.generate()
        ...
        if tosca_definitions_version:
            if not self.is_tosca_definitions_version_file():
                ...
                if self.configuration.get(ALLOY, 'open_tosca_definitions_version') == True:
                    if tosca_definitions_version == 'tosca_simple_yaml_1_1':
                        # NOTE: This is a temporary workaround, which would be removed later.
                        self.generate(Alloy.OPEN, 'cloudnet/tosca_simple_yaml_1_2 // ISSUE: Given value was ' + tosca_definitions_version)
                    else:
                        self.generate(Alloy.OPEN, 'cloudnet/' + tosca_definitions_version)
        ...
        #
        for generator_class in [DataTypeGenerator, ArtifactTypeGenerator, CapabilityTypeGenerator, RequirementTypeGenerator, RelationshipTypeGenerator, InterfaceTypeGenerator, NodeTypeGenerator, GroupTypeGenerator, PolicyTypeGenerator, TopologyTemplateGenerator]:
