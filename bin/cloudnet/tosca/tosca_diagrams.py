...
class ToscaDiagramGenerator(Generator):
    ...
    def generation(self):
        ...
        for node_name, node_yaml in node_templates.items():
            ...
            for requirement in syntax.get_requirements_list(node_yaml):
                for requirement_name, requirement_yaml in requirement.items():
                    ...
                    if requirement_yaml:
                        requirement_capability = syntax.get_requirement_capability(node_type_requirements.get(requirement_name))
                        if requirement_capability == None:
                            self.error(requirement_name + ': capability undefined')
                            continue
                        requirement_node = syntax.get_requirement_node_template(requirement_yaml)
                        ...
                        if requirement_node_template == None:
                            ...
                            continue
                        for capability_name, capability_yaml in syntax.get_capabilities(self.type_system.merge_node_type(syntax.get_type(requirement_node_template))).items():
                            if self.type_system.is_derived_from(syntax.get_capability_type(capability_yaml), requirement_capability):
                                capability_found = True
                                break
                        if capability_found:
                            target_capability_ids[id(requirement)] = self.get_node_name_id(requirement_node) + '_capability_' + normalize_name(capability_name)
                            ...
                            self.error(' capability of type "' +  requirement_capability + '" not found')
        ...
        for node_name, node_yaml in node_templates.items():
            ...
            self.generate('      label=""')
            self.generate('      ', node_name_id, '[label="', node_name, ': ', short_type_name(node_type),'|\l\l\l\l" shape=record style=rounded]', sep='')
            for capability_name, capability_yaml in syntax.get_capabilities(merged_node_type).items():
                if capability_name != 'feature' or node_name in show_feature_capabilities:
                    self.generate('      ', node_name_id, '_capability_', normalize_name(capability_name), '[label="', capability_name, '" shape=cds style=filled fillcolor=orange]', sep='')
                    self.generate('      ', node_name_id, '_capability_', normalize_name(capability_name), ' -- ', node_name_id, sep='')
            for requirement_name, requirement_yaml in syntax.get_requirements_dict(merged_node_type).items():
                if requirement_name != 'dependency' or node_name in show_dependency_requirements:
                    self.generate('      ', node_name_id, '_requirement_', normalize_name(requirement_name), '[label="', requirement_name, '" shape=cds style=filled fillcolor=turquoise]', sep='')
                    self.generate('      ', node_name_id, ' -- ', node_name_id, '_requirement_', normalize_name(requirement_name), sep='')
            self.generate('    }')
            ...
            for requirement in syntax.get_requirements_list(node_yaml):
                for requirement_name, requirement_yaml in requirement.items():
                    ...
                    if capability_id != None:
                        self.generate('    ', node_name_id, '_requirement_', normalize_name(requirement_name), ' -- ', capability_id, '[style=dotted]', sep='')
        ...
        if substitution_mappings != None:
            ...
            for requirement_name, requirement_yaml in syntax.get_substitution_mappings_requirements(substitution_mappings).items():
                if requirement_yaml:
                    ...
                    self.generate('  ', requirement_name_id, '[label="', requirement_name, '" shape=cds style=filled fillcolor=turquoise]', sep='')
                    self.generate('  ', normalize_name(requirement_yaml[0]), '_requirement_', normalize_name(requirement_yaml[1]), ' -- ', requirement_name_id, '[style=dotted]', sep='')
