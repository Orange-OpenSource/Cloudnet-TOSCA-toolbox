...
DEFAULT_CONFIGURATION[NWDIAG] = {
    ...
    'linkable_capability_types': [
        'tosca.capabilities.network.Linkable'
    ],

    # Port capability types.
    'bindable_capability_types': [
...
class NwdiagGenerator(Generator):
    ...
    def generation(self):
        ...
        topology_template = self.tosca_service_template.get_yaml().get(syntax.TOPOLOGY_TEMPLATE)
    ...
    def generate_network_diagram(self, topology_template):
        ...
        if substitution_mapping != None:
            ...
            if substitution_mapping_node_type != None:
                ...
                for requirement_name, requirement_value in syntax.get_substitution_mappings_requirements(substitution_mapping).items():
                    ...
                    if capability in self.configuration.get(NWDIAG, 'linkable_capability_types'):
        ...
        for node_name, node_yaml in node_templates.items():
            ...
            for cap_name, cap_def in node_type_type.get(syntax.CAPABILITIES, {}).items():
                cap_def_type = cap_def.get(syntax.TYPE) if isinstance(cap_def, dict) else cap_def
                if cap_def_type in self.configuration.get(NWDIAG, 'linkable_capability_types'):
            ...
            for requirement in syntax.get_requirements_list(node_yaml):
                for requirement_name, requirement_yaml in requirement.items():
                    ...
                    capability = syntax.get_requirement_capability(requirement_definition)
                    if capability in self.configuration.get(NWDIAG, 'linkable_capability_types'):
                        network_node = syntax.get_requirement_node_template(requirement_yaml)
                        # current node template is a node of the network node
                        get_network(network_node).nodes[node_name] = node_name
                    elif capability in self.configuration.get(NWDIAG, 'bindable_capability_types'):
                        binding_node = syntax.get_requirement_node_template(requirement_yaml)
        ...
        if len(networks) == 0: # no network node template found
            ...
            for network_name, network in networks.items():
                ...
                if network_node is None:
                    ...
                    network_label = self.resolve_attribute(network_node, network_repr, 'label', '\n')
                    network_label = network_label if network_label != '' else network_name
                    network_address = self.resolve_attribute(network_node, network_repr, 'address', '\n') #'', ')
                ...
                self.generate('    color = "%s"' % network_repr.get('color', 'lightblue'))
                self.generate('    textcolor = "%s"' % network_repr.get('textcolor', 'black'))
                ...
                if len(network.nodes) == 0: # no node found
                    ...
                    self.generate('    empty_network_%s[label="This network\nis empty!", shape=note];' % network_name)
                else:
                    ...
                    for port_name, node_name in network.nodes.items():
                        ...
                        port_address = self.resolve_attribute(port_node, port_repr, 'address', ', ')
                        ...
                        for binding in bindings:
                            ...
                            for binding in bindings:
                                ...
                                binding_label = self.resolve_attribute(binding_node, binding_repr, 'label', '\n')
                                binding_label = binding_label if binding_label != '' else binding
                                # generate the graphical node
                                # TODO: add other graphical attributes font, etc.
                                self.generate('    "%s"[label="%s", address="%s", shape="%s", color="%s", textcolor="%s", style="%s"%s];' \
                                    % ( binding, binding_label, port_address, \
                                        binding_repr.get('shape', 'box'), \
                                        binding_repr.get('color', 'white'), \
                                        binding_repr.get('textcolor', 'black'), \
                                        binding_repr.get('style', 'solid'), \
                                        self.get_icon_attribute(binding_node, binding_repr) \
                                    ))
                self.generate('  }')
            self.generate('}')
            self.close_file()
    ...
    def get_representation(self, node_template):
        ...
        while True:
            ...
            representation = self.configuration.get(NWDIAG, 'node_types').get(node_type_name)
    ...
    def resolve_attribute(self, node, representation, attribute_name, separator):
        ...
        for item in representation.get(attribute_name, []):
            ...
            if isinstance(value, dict):
                ...
                if input_name != None:
                    value = self.topology_template.get(syntax.INPUTS, {}).get(input_name, {}).get(syntax.DEFAULT, '')
    ...
    def get_icon_attribute(self, node, representation):
        ...
        target_directory = self.configuration.get(self.generator_configuration_id(), Generator.TARGET_DIRECTORY)
